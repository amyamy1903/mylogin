...
def hash_code(s, salt='login_app'):
    h = hashlib.sha256()
    s += salt
...
def login2(request):
    if request.method == "POST":
        ...
        if login_form.is_valid():
            ...
            try:
                #user = models.User.objects.get(name=username)
                user = mylogin.rbac.models.UserInfo.objects.get(name=username)
                if user.password == password:
                    return redirect('/index/')
                else:
                    message = "密码不正确！"
    ...
    if request.method == "POST":
        ...
        if login_form.is_valid():
            ...
            try:
                ...
                #print("user and password", user.username, user.password)
                if not user:
                    return render(request, "login.html", {'error': '用户名或密码错误！'})
                else:
...
def monitor(request):
    # try:
    #     render(request, 'monitor.html', locals())
    # except Exception as e:
    #     return render(request, 'error/error-400.html', {'message': e})
    return render(request, 'error/error-404.html', {'message': "网页不存在"})
